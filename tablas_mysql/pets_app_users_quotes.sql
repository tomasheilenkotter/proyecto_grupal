...
CREATE TABLE `users_quotes` (
  ...
  `quote_user_id` int NOT NULL,
  PRIMARY KEY (`user_quote_id`,`quote_user_id`),
  KEY `fk_users_has_quotes_quotes1_idx` (`quote_user_id`),
  KEY `fk_users_has_quotes_users1_idx` (`user_quote_id`),
  CONSTRAINT `fk_users_has_quotes_quotes1` FOREIGN KEY (`quote_user_id`) REFERENCES `quotes` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_users_has_quotes_users1` FOREIGN KEY (`user_quote_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
LOCK TABLES `users_quotes` WRITE;
